...
class LabelSmoothing(nn.Module):
    ...
    def __init__(self, smoothing=0.0):
        """
        Constructor for the LabelSmoothing nn.module.
...
class Block(nn.Module):
    ...
    def forward(self, x):
        ...
        #         f.write(json.dumps({"x2": x.detach().cpu().numpy()[0,0,:10].tolist()}) + '\n')
        x = x + h
...
class Encoder(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        with open(log_file, 'a') as f:
            f.write(json.dumps({"concat": concat.detach().cpu().numpy()[0,0,:10].tolist()}) + '\n')
        part_states, part_weights = self.part_layer(concat)
        
        # with open(log_file, 'a') as f:
        #     f.write(json.dumps({"part_states": part_states.detach().cpu().numpy()[0,0,:10].tolist()}) + '\n')
...
class Transformer(nn.Module):
    ...
    def forward(self, input_ids):
        ...
        # with open(log_file, 'a') as f:
        #     f.write(json.dumps({"embedding_output": embedding_output.detach().cpu().numpy()[0,0,:10].tolist()}) + '\n')
        part_encoded = self.encoder(embedding_output)
        return part_encoded

class t_VisionTransformer(nn.Module):
    def __init__(self, config, img_size=224, num_classes=21843, smoothing_value=0, zero_head=False):
        super(t_VisionTransformer, self).__init__()
    ...
    def forward(self, x, labels=None):
        # with open(log_file, 'a') as f:
        #     f.write(json.dumps({"x": x.detach().cpu().numpy()[0,0,0,:10].tolist()}) + '\n')
        part_tokens = self.transformer(x) # 4, 13, 768
        
        # with open(log_file, 'a') as f:
        #     f.write(json.dumps({"part_tokens": part_tokens.detach().cpu().numpy()[0,0,:10].tolist()}) + '\n')
