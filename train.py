...
log_file = None
# task_name = "overlap"
# log_file = os.path.join("/home/aiuser/TransFG/logs", datetime.datetime.now(tz).strftime("%m-%d-%H-%M") + task_name + ".jsonl")
...
def setup(args):
    ...
    model = VisionTransformer(config, args.img_size, zero_head=True, num_classes=num_classes, smoothing_value=args.smoothing_value, vit=args.vit)
...
def valid(args, model, test_loader, global_step):
    ...
    val_accuracy = jittor.detach(accuracy_jittor).cpu().numpy()
    ...
    with open(log_file, 'a') as f:
        f.write(json.dumps({"time": datetime.datetime.now(tz).strftime("%m-%d-%H-%M"), "val_accuracy": accuracy, "global_step": global_step}) + '\n')
...
#                                       optimizers=optimizer,
    ...
    while True:
        ...
        for x, y in train_loader:
            ...
            try:
                ...
                None
            loss, logits = model(x, y, args=args)
            loss = loss.mean()
            ...
            if True:
                losses.update((loss*args.gradient_accumulation_steps).numpy())
                if args.fp16:
                    # jittor.nn.utils.clip_grad_norm_(amp.master_params(optimizer), args.max_grad_norm)
                    None
                else:
        ...
        with open(log_file, 'a') as f:
            f.write(json.dumps({"time": datetime.datetime.now(tz).strftime("%m-%d-%H-%M"), "train_accuracy": accuracy, "global_step": global_step}) + '\n')
    ...
    with open(log_file, 'a') as f:
        f.write(json.dumps({"time": datetime.datetime.now(tz).strftime("%m-%d-%H-%M"), "total_time": (end_time - start_time) / 3600}) + '\n')
...
def main():
    ...
    parser.add_argument('--contrastive_loss', type=int, default=1)
    ...
    global log_file
    log_file = os.path.join("/home/aiuser/TransFG/logs", datetime.datetime.now(tz).strftime("%m-%d-%H-%M-") + args.name + ".jsonl")
